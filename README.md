...
A 股桌面盯盘与量化研究工具，基于 Electron。业务代码只使用 Node.js 内置模块，运行时无 npm 依赖。

> 本项目是研究与信息整理工具，不构成投资建议。行情、资讯、搜索结果与模型输出均可能延迟、缺失或错误。软件不会自动下单。
...
见 [RELEASE_NOTES_v1.9.3.md](RELEASE_NOTES_v1.9.3.md)。
